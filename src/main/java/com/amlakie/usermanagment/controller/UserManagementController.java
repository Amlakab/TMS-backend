...
import com.amlakie.usermanagment.service.EmailService;
import com.amlakie.usermanagment.service.OTPService;
import com.amlakie.usermanagment.service.UserManagementService;
...
public class UserManagementController {
    ...
    public ReqRes otp(@RequestBody ReqRes request) {
        return otpService.generateOTP(request.getEmail());
    }

    @PostMapping("/auth/verify-otp")
    public ReqRes verifyOtp(@RequestBody ReqRes request) {
        ReqRes response = new ReqRes();
       try {
           Optional<OurUsers> optionalUser = usersRepo.findByEmail(request.getEmail());
           if (optionalUser.isPresent()) {
               ...
               response.setMessage("user not found otp");
               response.setStatus(500);
           }
       }catch (Exception e){
